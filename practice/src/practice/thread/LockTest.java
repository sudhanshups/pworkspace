...
/*    displayQueueLock is created to make
    ...
    to be released by t1 as readRecord method uses readQueueLock not displayQueueLock.*/
    ...
    public void displayRecord(Object document) {
        ...
        try {
            ...
            System.out.println(Thread.currentThread().getName() + ": TestResource: display a Job" +
                    " during " + (duration / 1000) + " seconds :: Time - " + new Date());
            Thread.sleep(duration);
        } catch (InterruptedException e) {
            e.printStackTrace();
        } finally {
            System.out.printf("%s: The document has been dispalyed\n", Thread.currentThread().getName());
            displayLock.unlock();
        }
    }
